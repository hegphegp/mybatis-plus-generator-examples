...
public class MysqlGenerator {
    ...
    public static void generatorCode(String url, String username, String password, String author, String outputDir, String tableName, String entityBaseName, IdType idType,
        ...
        strategy.setEntityTableFieldAnnotationEnable(false); // 字段是否生成映射注解，实体类是否生成 @TableName("") 注解这个被改了，实体类的前面都生成注解
        ...
        focList.add(new FileOutConfig("/templates/pojo.java.ftl") {
            @Override
            public String outputFile(TableInfo tableInfo) { // 自定义输入文件名称
                String classPath = packagePath.endsWith(".")? packagePath+module:packagePath+"."+module;
                classPath = classPath.replaceAll("\\.", separator);
                if (System.getProperty("os.name").toLowerCase().contains("win")) {
                    return outputDir+"\\java\\"+classPath+"\\pojo\\"+entityBaseName+".java";
                } else { // linux平台
                    return outputDir+"/java/"+classPath+"/pojo/"+entityBaseName+".java";
                }
            }
